...
/*
Plugin Name: Woocommerce Product List
Plugin URI: https://wordpress.com/
Description: Woocommerce Product List Table
Version: 1.0
...
class Posts_List extends WP_List_Table
{
	...
	public static function get_my_post($per_page = 5, $page_number = 1)
	{
	  ...
	  GROUP BY p.ID,p.post_title";
		...
		if (!empty($_REQUEST['s'])) {
			...
			$sql .= " AND (p.post_title LIKE '%{$search}%' OR p.post_content LIKE '%{$search}%')";
		}
	...
	public static function record_count()
	{
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}posts where post_type = 'product'";
	...
	function get_columns()
	{
		$columns = [
			...
			'product_category' => __('Categories', 'sp'),
			'sku'    => __('SKU', 'sp'),
			'price'    => __('Price', 'sp'),
...
class SP_Plugin
{
	...
	public function __construct()
	{
		add_filter('set_screen_option', [__CLASS__, 'set_screen'], 10, 5);
